...
void InitMP3s(void)
{
	...
	cleveland_SND = BASS_StreamCreateFile(FALSE,"cleveland.mp3",0,0,0);
}
						   ...
						   void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_TIMER_TICK:
			...
			if(flag == 0) //only for cards with no place
				random();
...
void mouse_click()
{
	 ...
	 counter_3++; // count moves
			
			if(counter_3 == 10) // minus 5 points for every 10 clicks which mean 5 turns of card pairs clicking
			{
				if(point>0)
				{
					point--;
					...
					if(game_col != game_col_1 || game_row != game_row_1) //prevent the player to press the same card
							{
								...
								if(counter==1) //first card choose
								{
								...
								if(counter==2) //second card choose
								{
							...
							void *callbackData, int eventData1, int eventData2)
{
	...
	switch(event)
	{
		case EVENT_LEFT_CLICK:
			...
			mouse_click();
			if(flag_2==0 && counter == 0) //if the two cards that are open arent the same than close them
				{	
						BASS_ChannelPlay(cleveland_SND,TRUE);
						Delay(1);
